...
public class SearchService {
    ...
    private List<Map<String, Object>> getCategoryAggResult(Aggregation aggregation) {
        ...
        //获取桶的集合转化成List<Map<String,Object>>集合
        return terms.getBuckets().stream().map(bucket -> {
            Map<String, Object> map=new HashMap<>();
            //获取桶中的分类id
    ...
    public Goods buildGoods(Spu spu) throws IOException {
        ...
        skus.forEach(sku -> {
            ...
            //获取sku中的图片，数据库中的图片可能是多张，多张以“,”进行分割，所以我们以逗号来切割返回图片数组，获取第一张
            map.put("iamge",StringUtils.isBlank(sku.getImages()) ? "" : StringUtils.split(sku.getImages(),","));
            skuMapList.add(map);
        });
        ...
        params.forEach(param->{
            //判断规格参数的类型是否是通用的规格参数
            if (param.getGeneric()){
                //如果是通用的规格参数，从genericSpecMap中获取规格参数值
                ...
                //如果是特殊的规格参数，从SpecialSpecMap中获取规格参数值
